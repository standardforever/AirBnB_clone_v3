...
def handle_places(city_id=None, place_id=None):
    '''HTTP method handler for the places endpoint.
    '''
    handlers = {
        'GET': get_places,
        'DELETE': remove_place,
...
def get_places(city_id=None, place_id=None):
    '''Gets place by id or all places from city with the given id.
    '''
    if city_id:
...
def find_places():
    '''Query places based on State, City, or Amenity ids.
    '''
    data = request.get_json()
    if type(data) is not dict:
        raise BadRequest(description='Not a JSON')
